...
# --- 1. CONFIG & STYLING (V10.2 GOLDEN CLOUD) ---
...
# --- 3. DATA ENGINE (CLOUD-SAFE BULK FETCH) ---
@st.cache_data(ttl=3600)
def get_unified_data(tickers):
    if not tickers: return {}
    
    # CLOUD FIX: Single bulk download to avoid IP throttling
    try:
    ...
    for t in tickers:
        try:
            # Handle Multi-Index flattening for bulk data
            ...
            is_cef = t in HARDCODED_CEFS or any(kw in sumry for kw in ["closed-end", "statutory trust", "management investment company"])
            sector = "Closed-End Fund" if is_cef else (info.get('quoteType', 'Other') if info.get('quoteType') == "ETF" else info.get('sector', 'Other'))
...
# --- 5. MAIN UI ---
st.markdown(f'<div class="app-branding">Income Portfolio Tracker by QTI (v10.2)</div>', unsafe_allow_html=True)
...
with t_edit:
    ...
    with st.form("entry", clear_on_submit=True):
        ...
        ns = c2.number_input("Shares", min_value=0.0); nc = c3.number_input("Avg Cost", min_value=0.0)
    ...
    st.download_button(f"💾 SAVE {strip_ext(active).upper()} CSV", data=csv_data, file_name=f"{strip_ext(active)}.csv", mime='text/csv')
    ...
    for idx, row in df_edit.iterrows():
        ...
        r[1].write(f"**{row['Ticker']}**"); r[2].write(f"{row['Shares']:,.2f}"); r[3].write(f"${row['Avg Cost']:,.2f}")
...
with t_dash:
    ...
    if not df.empty:
        with st.spinner("Syncing Live Data..."): meta = get_unified_data(df['Ticker'].unique().tolist())
        ...
        def draw_donut(pdf, val_col, label_col, total_overall, hole=0.5):
            def agg(g):
                s_g = g.sort_values(val_col, ascending=False).head(15); b = "<br>".join([f"• {t}: <b>${amt:,.2f}</b>" for t, amt in zip(s_g['Ticker'], s_g[val_col])])
            ...
            f = go.Figure(data=[go.Pie(labels=sum_df[label_col], values=sum_df['Val'], hole=hole, marker=dict(colors=['#2ecc71', '#f1c40f', '#e74c3c'] if label_col=='Safety' else px.colors.qualitative.Pastel), customdata=sum_df['Hover'], hovertemplate="<b>%{customdata}</b><extra></extra>")])
            f.update_layout(height=600, margin=dict(t=30, b=80), hoverlabel=HOVER_STYLE)
            st.plotly_chart(f, use_container_width=True)

        with c1: st.subheader("Dynamic Safety Rating"); draw_donut(df, "Income", "Safety", total_inc, hole=0.6)
        with c2:
        ...
        with c3:
            ...
            draw_donut(df, "Portfolio Value" if v_t == "Portfolio Value" else "Income", "Sector", total_mv if v_t == "Portfolio Value" else total_inc)
        ...
        if cal_list:
            ...
            def m_stats(g):
                s_g = g.sort_values('Income', ascending=False).head(15); b = "<br>".join([f"• {t}: <b>${amt:,.2f}</b>" for t, amt in zip(s_g['Ticker'], s_g['Income'])])
        ...
        st.write("---"); st.subheader("📋 Detailed Analytics")
        df_disp = df[['Ticker', 'Sector', 'Safety', 'Price', 'Yield', 'Portfolio Value', 'Income']].sort_values('Portfolio Value', ascending=False)
