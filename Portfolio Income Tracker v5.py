...
st.markdown("""
    ...
    .stButton > button, .stDownloadButton > button { 
        ...
        transition: all 0.1s ease !important; box-shadow: 0 4px #999;
    }
    .stButton > button:active, .stDownloadButton > button:active {
        box-shadow: 0 0px #666 !important; transform: translateY(3px) !important;
    }
    div[data-testid="stFormSubmitButton"] button { background-color: #2ecc71 !important; color: white !important; border: none !important; }
    div.stDownloadButton button { background-color: #3498db !important; color: white !important; border: none !important; }
...
def get_unified_data(tickers):
    ...
    for t in tickers:
        try:
            ...
            curr_price = float(t_prices['Close'].dropna().iloc[-1])
            div_history = t_prices['Dividends'].dropna()
            div_history = div_history[div_history > 0]
            tk = yf.Ticker(t);
            info = tk.info
            div_rate = float(info.get('dividendRate', 0) or 0)
            if div_rate == 0 and not div_history.empty:
                one_year_ago = datetime.now() - timedelta(days=365)
                div_rate = float(div_history[div_history.index > one_year_ago].sum())
            ...
            meta[t] = {'price': curr_price, 'div': div_rate, 'freq': 12 if len(div_history) > 6 else 4,
                       'ex_date': int(div_history.index[-1].timestamp()) if not div_history.empty else None,
                       'sector': sector, 'safety': tier}
            time.sleep(0.1)
        except:
...
st.markdown(f'<div class="app-branding">Income Portfolio Tracker by QTI (v10.9)</div>', unsafe_allow_html=True)
active = st.session_state.get('active_portfolio_name')

# --- 5. STARTUP EXPERIENCE (RE-ENGINEERED) ---
if not active:
    st.markdown('<div class="master-title">Welcome to Income Tracker</div>', unsafe_allow_html=True)

    col_guide, col_actions = st.columns([1.2, 1])

    with col_guide:
        st.markdown("""
        ### 🚀 Getting Started
        To begin tracking your dividend income, you need to provide your stock data in a specific format.

        #### **How to create your Portfolio.csv:**
        1. **Excel or Google Sheets:** Open a new sheet and create three columns in the first row:
            * **Ticker:** (e.g., AAPL, SCHD, O)
            * **Shares:** The number of units you own.
            * **Avg Cost:** Your average purchase price per share.
        2. **Save as CSV:** In Excel, go to *File > Save As* and select **CSV (Comma Delimited)**.
        3. **Upload:** Drag and drop that file into the sidebar on the left.
        """)

    with col_actions:
        st.markdown("### 🛠️ Setup Options")

        # Option 1: Template Download
        template_df = pd.DataFrame(columns=["Ticker", "Shares", "Avg Cost"],
                                   data=[["SCHD", 100.0, 75.50], ["O", 50.0, 62.00]])
        st.download_button(
        ...
        st.caption("Download a pre-formatted file to fill in your data.")

        st.write("")

        # Option 2: Load Sample
        if st.button("📈 Load Default Sample Portfolio", use_container_width=True):
            if os.path.exists("Sample Portfolio.csv"):
                ...
                st.error("Sample Portfolio file not found on server.")
        st.caption("Explore the dashboard immediately using our demo data.")
    st.stop()
...
with t_edit:
    ...
    existing_tickers = sorted(df_edit['Ticker'].unique().tolist())
    sel_ticker = st.selectbox("Select existing Ticker or type new symbol:", [""] + existing_tickers,
                              format_func=lambda x: "--- Create New ---" if x == "" else x)

    final_ticker = sel_ticker
    if sel_ticker == "":
        new_tk = st.text_input("Type new Ticker symbol:").upper().strip()
        final_ticker = new_tk

    curr_shares, curr_cost = 0.0, 0.0
    if final_ticker in df_edit['Ticker'].values:
        row = df_edit[df_edit['Ticker'] == final_ticker].iloc[0]
        curr_shares, curr_cost = float(row['Shares']), float(row['Avg Cost'])

    with st.form("command_form", clear_on_submit=True):
        c1, c2 = st.columns(2)
        ns = c1.number_input("Shares", min_value=0.0, value=curr_shares, format="%.2f")
        nc = c2.number_input("Avg Cost", min_value=0.0, value=curr_cost, format="%.2f")
        if st.form_submit_button("COMMIT CHANGES TO PORTFOLIO"):
            if final_ticker:
                if ns <= 0:
                    st.session_state.portfolios[active] = df_edit[df_edit['Ticker'] != final_ticker]
                    st.toast(f"🗑️ {final_ticker} removed.", icon="⚡")
                elif final_ticker in df_edit['Ticker'].values:
                    df_edit.loc[df_edit['Ticker'] == final_ticker, ['Shares', 'Avg Cost']] = [ns, nc]
                    st.toast(f"✅ {final_ticker} updated.", icon="⚡")
                else:
                    st.session_state.portfolios[active] = pd.concat(
                        [df_edit, pd.DataFrame([{"Ticker": final_ticker, "Shares": ns, "Avg Cost": nc}])],
                        ignore_index=True)
                    st.toast(f"🚀 {final_ticker} added.", icon="⚡")
                time.sleep(1);
                st.rerun()

    st.divider()
    st.subheader("📋 Delete Tickers")
    selected_for_delete = st.multiselect("Select Tickers to Remove:", existing_tickers)
    if selected_for_delete and st.button(f"🗑️ DELETE SELECTED ({len(selected_for_delete)})", type="primary"):
        st.session_state.portfolios[active] = df_edit[~df_edit['Ticker'].isin(selected_for_delete)]
        st.toast("🗑️ Tickers removed.", icon="🔥");
        time.sleep(1);
        st.rerun()

    st.write("")
    st.download_button(f"💾 SAVE CURRENT PORTFOLIO", data=df_edit.to_csv(index=False).encode('utf-8'),
                       file_name=f"{strip_ext(active)}.csv", mime='text/csv')

    df_edit_sorted = df_edit.sort_values(by="Ticker")
    edit_html = "<div class='html-table-container'><table class='gold-table'><thead><tr><th>Ticker</th><th>Shares</th><th>Avg Cost</th><th>Total Basis</th></tr></thead><tbody>"
    for _, row in df_edit_sorted.iterrows():
        edit_html += f"<tr><td class='tk-bold'>{row['Ticker']}</td><td>{row['Shares']:,.2f}</td><td>${row['Avg Cost']:,.2f}</td><td>${(row['Shares'] * row['Avg Cost']):,.2f}</td></tr>"
    st.markdown(edit_html + "</tbody></table></div>", unsafe_allow_html=True)
...
with t_dash:
    ...
    if not df.empty:
        ...
        html = "<div class='html-table-container'><table class='gold-table'><thead><tr><th>Ticker</th><th>Sector</th><th>Safety</th><th>Price</th><th>Yield</th><th>Portfolio Value</th><th>Income</th></tr></thead><tbody>"
